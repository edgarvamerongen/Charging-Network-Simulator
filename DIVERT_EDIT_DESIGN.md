...
2. **Suitable airports only** — manual divert targets are restricted to the same landable bar as the auto-divert (paved ≥300m, open). Drag snaps to the nearest **suitable** airport; the ALT typeahead is filtered to suitable. (This method of checking for suitable airports will soon be replaced by a more accurate divert fitting — the **performance-engine S5** per-aircraft runway suitability, `airport_suitability(plane, airport)` from `field_performance`; see `docs/performance-engine.md §7` + `performance-engine-step2-plan.md` S5. When S5 lands, `divertFor()` / `nearestSuitable` should gate on **per-aircraft** suitability instead of the generic paved-≥300m rule — and the two efforts must not edit `airport_alternates.py` semantics independently.)
