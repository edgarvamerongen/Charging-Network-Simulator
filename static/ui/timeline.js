...
(function () {
  ...
  function render() {
    ...
    if (!folder.length || !SC()) { rows += `<div class="empty">Add a route to the network to see its charging sessions on the day.</div>`; $('#drawerSub').textContent = 'no flights yet'; }
      ...
      const g = SC().runGlobal(); const prof = loadProfile(g.lanes, foc); const flights = folder.reduce((s, t) => s + D().flightsPerDay(t), 0);
      const path = prof.load.map((v, i) => { const y = (40 - (prof.peak ? v / prof.peak * 36 : 0)).toFixed(1); return `L${i} ${y} L${i + 1} ${y}`; }).join(' ');
      rows += `<div class="grow load"><div class="lab">${foc ? foc + ' load' : 'Network load'}<small>peak ${UI.fmt.kw(prof.peak)}</small></div><div class="track">${bare}<svg viewBox="0 0 ${prof.N} 40" preserveAspectRatio="none"><path d="M0 40 ${path} L${prof.N} 40 Z"/></svg></div></div>`;
      if (S.lanes === 'fleet') {
        ...
        $('#drawerSub').textContent = `${g.lanes.length} aircraft · ${flights % 1 ? flights.toFixed(1) : flights} flights / day · peak load ${UI.fmt.kw(prof.peak)}`;
      } else {
  ...
  document.addEventListener('pointerup', e => { if (!drag) return; const [tripId, k] = drag.key.split(':'); const nt = Math.max(H0 + 60, Math.min(H1, Math.round((drag.takeoff + (drag.dm || 0)) / 5) * 5)); drag.el.style.transform = '';
    ...
    drag = null; });
  document.addEventListener('click', e => { const t = e.target.closest('#laneSeg button,#depSw,#focChip,#drawerHead'); if (!t) return;
    if (t.closest('#laneSeg')) { S.lanes = t.dataset.lanes; render(); return; }
    if (t.id === 'depSw') { S.showDep = !S.showDep; render(); return; }
