...
(function () {
  ...
  const FIELDS = [
    { key: 'landingReserve', title: 'Landing reserve', desc: 'Minimum state of charge on landing. Reduces the usable range of every leg.', sliders: [{ f: 'minLandingSoc', label: 'Reserve', min: 5, max: 40, step: 5, to: v => v / 100, from: v => pct(v), unit: '%' }] },
    ...
    { key: 'climbModel', title: 'Climb overhead', desc: 'Net climb-minus-descent energy per leg, ramping to full by a fraction of the catalog range (CLIMB_ENERGY_MODEL.md).', sliders: [{ f: 'overheadPct', label: 'Overhead', min: 0, max: 20, step: 1, to: v => v / 100, from: v => pct(v), unit: '% of battery' }, { f: 'satFrac', label: 'Saturates at', min: 5, max: 30, step: 1, to: v => v / 100, from: v => pct(v), unit: '% of range' }], example: true },
    { key: 'sidStarPadding', title: 'SID / STAR padding', desc: 'Fixed kilometres added to each leg for terminal procedures. Not applied to VFR aircraft.', sliders: [{ f: 'km', label: 'Per leg', min: 0, max: 40, step: 1, to: v => v, from: v => v, unit: 'km' }] },
    { key: 'routingPadding', title: 'Airways padding', desc: 'Multiplier on great-circle distance for airways routing. Off by default; SID / STAR padding is the preferred model.', sliders: [{ f: 'factor', label: 'Factor', min: 100, max: 130, step: 1, to: v => v / 100, from: v => Math.round(v * 100), unit: '%', show: v => (v / 100).toFixed(2) + '×' }] },
    { key: 'chargeTaper', title: 'Charge taper', desc: 'Real charging curve: full power up to the knee, then an exponential roll-off toward the floor.', sliders: [{ f: 'threshold', label: 'Knee', min: 50, max: 95, step: 5, to: v => v / 100, from: v => pct(v), unit: '% SoC' }, { f: 'taperPower', label: 'Floor', min: 10, max: 60, step: 5, to: v => v / 100, from: v => pct(v), unit: '% of peak' }] },
    ...
    { key: 'chargerEfficiency', title: 'Charger efficiency', desc: 'Grid-to-cell efficiency. Inflates grid energy and peak load versus what the aircraft receives.', sliders: [{ f: 'value', label: 'Efficiency', min: 70, max: 100, step: 1, to: v => v / 100, from: v => pct(v), unit: '%' }] },
  ];
  function example() {
    ...
    return `<div class="mh"><h3>Model settings</h3><button class="tb icon" data-modal="close"><svg class="ic"><use href="#i-x"/></svg></button></div>
      <div class="mb ms">
        ...
        <div class="msr"><div class="row" style="align-items:flex-start;gap:10px"><span style="width:26px"></span><div style="flex:1"><div class="name" style="font-size:13px">Charge tariff</div><div class="hint" style="margin-top:2px">Price per charged kWh for the revenue figures.</div>
          <div class="msl"><span class="cap">Tariff</span><input type="range" min="10" max="200" step="5" value="${Math.round(rate * 100)}" data-ms="rate"><b class="num" data-ms-val="rate">€${(+rate).toFixed(2)} / kWh</b></div></div></div></div>
  ...
  document.addEventListener('click', e => {
    ...
    if (e.target.closest('#setBtn')) { open(); }
  });
