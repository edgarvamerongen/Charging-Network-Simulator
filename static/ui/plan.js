...
(function () {
  ...
  function aircraftHtml(p, reach, fits, pickHtml) {
    ...
    return `<div class="sec acsec"><div class="lbl"><span class="cap">Aircraft</span><span class="ac-tools"><button class="fbtn" data-act="acFilters">${summary} <span class="ch">▾</span></button><button class="lnk" data-act="pick">${S.picking ? 'Close' : 'Change'}</button>${pop}</span></div>
      ${pickHtml}
      <div class="ac-stage ov" style="background-image:url('/pics/${esc(p.image || '')}')"><button class="ac-arrow l" data-act="acPrev" title="Previous aircraft" ${vis.length > 1 ? '' : 'disabled'}>‹</button><button class="ac-arrow r" data-act="acNext" title="Next aircraft" ${vis.length > 1 ? '' : 'disabled'}>›</button>
      ...
      <div class="meta num">Reach ${fmt.r(fmt.km(reach))} of ${fmt.r(fmt.km(p.range_km || 0))} ${fmt.ukm()} · ${usage ? Math.round(usage) + ' kWh/100 km' : ''}${climb.applies ? ' +' + Math.round(climb.eMaxKwh) + ' climb' : ''}${S.availOverride != null ? ' · <b>override</b>' : ''}</div></div>`;
  }
