...
(function () {
  ...
  function aircraftHtml(p, reach, fits, pickHtml) {
    ...
    return `<div class="sec acsec"><div class="lbl"><span class="cap">Aircraft</span><span class="ac-tools"><button class="fbtn" data-act="acFilters">${summary} <span class="ch">▾</span></button><button class="lnk" data-act="pick">${S.picking ? 'Close' : 'Change'}</button>${pop}</span></div>
      ${pickHtml}
      ${S.acOverlay ? `<div class="ac-stage ov" style="background-image:url('/pics/${esc(p.image || '')}')"><button class="ac-arrow l" data-act="acPrev" title="Previous aircraft" ${vis.length > 1 ? '' : 'disabled'}>‹</button><button class="ac-arrow r" data-act="acNext" title="Next aircraft" ${vis.length > 1 ? '' : 'disabled'}>›</button>
        <span class="ac-st"><i class="dot s-${stCls}"></i>${esc(cap1(st) || 'Status unknown')}${cert}</span><span class="seg ac-rg">${['VFR', 'IFR'].map(r => `<button data-act="acRegime" data-v="${r}" class="${p.regime === r ? 'on' : ''}" ${hasR(r) ? '' : 'disabled'}>${r}</button>`).join('')}</span>
        <div class="ac-ov"><div class="row" style="justify-content:space-between;align-items:baseline"><span class="nm">${esc(p.name)}</span><span class="sub num">${esc(p.oem || '')}${vis.length && idx >= 0 ? ' · ' + (idx + 1) + ' of ' + vis.length : ''}</span></div>
          <div class="row6"><div><div class="k">Seats</div><div class="v num">${p.seats ?? '—'}</div></div><div><div class="k">Battery</div><div class="v num">${p.battery_kwh ?? '—'}<small>kWh</small></div></div><div><div class="k">Range</div><div class="v num">${fmt.r(fmt.km(p.range_km || 0))}<small>${fmt.ukm()}</small></div></div><div><div class="k">Cruise</div><div class="v num">${p.speed_kmh ?? '—'}<small>km/h</small></div></div><div><div class="k">Max chg</div><div class="v num">${p.max_charge_kw ?? '—'}<small>kW</small></div></div><div><div class="k">Reach</div><div class="v num">${fmt.r(fmt.km(reach))}<small>${fmt.ukm()}</small></div></div></div></div></div>
        <div class="row" style="justify-content:flex-end;margin-top:8px"><button class="lnk" data-act="acEdit">${S.availOverride != null ? 'Reset override' : 'Edit for this flight'}</button></div>`
      ...
      ${S.acOverlay ? '' : `<div class="ac-grid"><div><div class="k">Seats</div><div class="v num">${p.seats ?? '—'}</div></div><div><div class="k">Battery</div><div class="v num">${p.battery_kwh ?? '—'}<small>kWh</small></div></div><div><div class="k">Range</div><div class="v num">${fmt.r(fmt.km(p.range_km || 0))}<small>${fmt.ukm()}</small></div></div><div><div class="k">Cruise</div><div class="v num">${p.speed_kmh ?? '—'}<small>km/h</small></div></div><div><div class="k">Max charge</div><div class="v num">${p.max_charge_kw ?? '—'}<small>kW</small></div></div><div><div class="k">Reach</div><div class="v num">${fmt.r(fmt.km(reach))}<small>${fmt.ukm()}</small></div></div></div>`}
