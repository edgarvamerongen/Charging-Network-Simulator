    ...
    return `<div class="sec acsec"><div class="lbl"><span class="cap">Aircraft</span><span class="ac-tools"><button class="fbtn" data-act="acFilters">${summary} <span class="ch">▾</span></button><button class="lnk" data-act="pick">${S.picking ? 'Close' : 'Change'}</button>${pop}</span></div>
      ...
      ${S.acOverlay ? `<div class="ac-stage ov" style="background-image:url('/pics/${esc(p.image || '')}')"><button class="ac-arrow l" data-act="acPrev" title="Previous aircraft" ${vis.length > 1 ? '' : 'disabled'}>‹</button><button class="ac-arrow r" data-act="acNext" title="Next aircraft" ${vis.length > 1 ? '' : 'disabled'}>›</button>
        ...
        <div class="ac-ov"><div class="nm"><span class="mk">${esc(p.oem || '')}</span> ${esc(UI.planeShort(p.name).replace(new RegExp('^' + (p.oem || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\s+', 'i'), ''))}</div>
          <div class="sp num">${[p.seats != null ? p.seats + ' seats' : '', p.battery_kwh != null ? p.battery_kwh + ' kWh' : '', p.range_km ? fmt.r(fmt.km(p.range_km)) + ' ' + fmt.ukm() : '', p.speed_kmh ? p.speed_kmh + ' km/h' : ''].filter(Boolean).join(' · ')}</div></div></div>
      ...
      <div class="meta num">${S.acOverlay ? 'Reach ' + fmt.r(fmt.km(reach)) + ' of ' + fmt.r(fmt.km(p.range_km || 0)) + ' ' + fmt.ukm() + ' · ' : ''}${usage ? Math.round(usage) + ' kWh/100 km' : ''}${climb.applies ? ' +' + Math.round(climb.eMaxKwh) + ' climb' : ''}${S.acOverlay ? '' : ' · reserve ' + resv + ' %'}${S.availOverride != null ? ' · <b>override</b>' : ''}</div></div>`;
  }
